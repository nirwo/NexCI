window.addEventListener("load", function () {
  ...
  function showError(message, context) {
    ...
    switch (context) {
      case "job-list":
        errorDiv = jobListError;
        break;
      case "build":
        errorDiv = buildErrorOutput;
        break;
      case "log":
        errorDiv = logErrorOutput;
        break;
    }
    if (errorDiv) {
  ...
  function loadDashboard() {
    console.log('>>> loadDashboard called'); // Add logging here
    fetchJobs();
  }
  ...
  function populateJobList(jobs) {
    ...
    function updateBuildStats(builds) {
      ...
      document.getElementById(
        "stat-last-build"
      ).textContent = `#${lastBuild.number}`;
      document
        .getElementById("stat-last-build")
      ...
      if (lastBuildStatus === "SUCCESS") {
        document
          .getElementById("stat-last-build")
          .parentElement.parentElement.classList.add("bg-success");
      } else if (lastBuildStatus === "FAILURE") {
        document
          .getElementById("stat-last-build")
          .parentElement.parentElement.classList.add("bg-danger");
      } else {
        document
          .getElementById("stat-last-build")
          .parentElement.parentElement.classList.add("bg-warning");
      }
      // Calculate average duration
      const buildsWithDuration = builds.filter((b) => b.duration);
      if (buildsWithDuration.length > 0) {
        const totalDuration = buildsWithDuration.reduce(
      ...
      if (improving) {
        trend = "↑";
        document
          .getElementById("stat-build-trend")
          .parentElement.parentElement.classList.remove(
            "bg-warning",
            "bg-danger"
          );
    ...
    // --- Build Insights ---
    async function fetchAndDisplayBuildInsights(
      jobFullName,
      buildsData = null
    ) {
      buildErrorOutput.style.display = "none";
      ...
      if (buildsData) {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Error fetching builds for chart: ${response.status}`
            );
          }
      ...
      try {
        ...
        if (
          !dataToProcess ||
          !dataToProcess.builds ||
          dataToProcess.builds.length === 0
        ) {
        ...
        const buildSuccessChartCanvas =
          document.getElementById("buildSuccessChart");
        ...
        const durationTrendArea = document.getElementById(
          "duration-trend-area"
        );
    ...
    function renderDurationTrendChart(builds) {
      ...
      const durations = builds.map((b) =>
        b.duration ? b.duration / 1000 / 60 : 0
      ); // Convert to minutes
    ...
    async function fetchAndDisplayLogs() {
      ...
      try {
        const response = await fetch(
          `/api/log?build_url=${encodeURIComponent(latestBuildUrl)}`
        );
        ...
        const data = await response.json();
        logContentElement.textContent =
          data.log_content || "Log content is empty.";
    ...
    function processLogLine(line, timestamp, steps, currentStep) {
      const startingStepRegex =
        /Starting (building|execution of|processing|checkout|fetching) (.*)/i;
      const completedStepRegex =
        /(Finished|Completed|Done): (.*) with result: (SUCCESS|FAILURE|UNSTABLE|ABORTED)/i;
      const buildStatusRegex =
        /Build (.*): (SUCCESS|FAILURE|UNSTABLE|ABORTED)/i;
    ...
    function createFallbackSteps(logContent, steps) {
      ...
      if (
        /Finished: FAILURE|BUILD FAILURE|FATAL ERROR|failed|exception occurred/i.test(
          logContent
        )
      ) {
        overallStatus = "FAILURE";
      } else if (
        /Finished: UNSTABLE|warnings|test failures/i.test(logContent)
      ) {
    ...
    function renderTimelineFromLogs(logContent) {
      ...
      if (!steps || steps.length === 0) {
        timelineContent.innerHTML =
          '<div class="alert alert-info">No timeline data could be extracted from the log.</div>';
        return;
      }
      ...
      steps.forEach((step, index) => {
        ...
        stepRow.innerHTML = `
                ...
                <div class="col-1"><span class="badge ${statusBadgeClass}">${
          step.status
        }</span></div>
            `;
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <p><strong>Status:</strong> <span class="badge ${statusBadgeClass}">${
          step.status
        }</span></p>
                            </div>
                        </div>
    ...
    function attachStepClickHandler(stepRow, index) {
      stepRow.addEventListener("click", () => {
        const detailsElement = document.querySelector(
          `.timeline-details[data-step-index="${index}"]`
        );
    ...
    document.addEventListener("click", (event) => {
      const jobList = getElement("job-list"); // Get job list first

      // Check if the click originated within the job list
      if (
        jobList &&
        event.target &&
        event.target.closest("#job-list") &&
        event.target.matches(".list-group-item")
      ) {
        event.preventDefault();

        // Now check for the other required elements since we are in the job list context
        ...
        if (!jobDetailsArea || !logDisplayArea || !selectedJobTitle) {
          console.error(
            "One or more required detail/log UI elements not found within the job list context."
          );
          return; // Exit if elements needed for job details are missing
        }
        ...
        selectedJobTitle.textContent = event.target.textContent.split(" (")[0]; // Update selected job title

        if (jobId) {
          fetchJobDetails(jobId, jobDetailsArea, logDisplayArea);
        } else {
          showError("Job ID not found for the selected item.", "details");
        }
      }
      // No 'else' needed here - if the click wasn't in the job list, we do nothing
    });
    ...
    if (fetchLogsBtn) {
      fetchLogsBtn.addEventListener("click", () => {
        if (latestBuildUrl) {
          ...
          showError(
            "Build information (including log URL) has not been loaded.",
            "log"
          );
        }
      });
    ...
    function create24HourExecutionGraph(builds) {
      try {
        ...
        const recentBuilds = builds.filter(
          (build) => build.timestamp >= oneDayAgo
        );
        ...
        if (recentBuilds.length === 0) {
          ...
          if (container) {
            container.innerHTML =
              '<div class="alert alert-info">No builds in the last 24 hours</div>';
          }
          return;
        }
            ...
            datasets: [
              {
                ...
                pointBackgroundColor: function (context) {
                  ...
                  switch (status) {
                    case "SUCCESS":
                      return "rgba(75, 192, 75, 1)"; // Green
                    case "FAILURE":
                      return "rgba(255, 99, 132, 1)"; // Red
                    case "UNSTABLE":
                      return "rgba(255, 205, 86, 1)"; // Yellow
                    case "ABORTED":
                      return "rgba(201, 203, 207, 1)"; // Grey
                    default:
                      return "rgba(54, 162, 235, 1)"; // Blue (in progress)
                  }
                },
                pointRadius: 6,
                pointHoverRadius: 8,
        ...
        if (container) {
          container.innerHTML =
            '<div class="alert alert-danger">Error creating chart: ' +
            error.message +
            "</div>";
        }
      }
